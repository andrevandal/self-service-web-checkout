...
5. A pinned bottom bar that always shows `N items` and the running subtotal. It exposes `View cart` when the cart is non-empty and a disabled `View cart` state when empty. Cart details are shown in a compact in-page panel when requested, listing each line's product/customization summary, line total, and a Remove action. Removing a line updates the count and subtotal immediately. The checkout action is present as a disabled/non-navigating affordance until spec 4 supplies its server flow.
