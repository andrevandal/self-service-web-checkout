...
`CheckoutScreen` owns checkout phase, pending order identity, terminal attempt state, failure copy, and confirmation timing. Its public contract is a cart snapshot plus `onCancel` and `onComplete` callbacks, so it can be tested without knowing how the menu renders. It does not compute trusted money or construct server persistence records beyond mapping cart selections to IDs. Each `createPendingOrder`/`runAttempt` invocation mints its own monotonic run token before doing any async work; every `await` boundary checks the token against the current run before writing state. A stale invocation (superseded by React's dev-mode double-effect-invoke, or by a fresh Retry tap) silently no-ops instead of racing or wedging the phase, which is the only correct way to guard async state updates against multiple overlapping invocations of the same effect.
